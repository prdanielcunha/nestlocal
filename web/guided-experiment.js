...
export function renderGuidedExperiment({experiments=[],snapshot={},canManage=false,t,esc}){
  ...
  if(active){
    ...
    const reason=!state.canManage?t('experimentManagerOnly'):t('experimentNeedsData').replace('{w}',String(Math.max(0,5-state.whatsapp))).replace('{p}',String(Math.max(0,5-state.phone)));
