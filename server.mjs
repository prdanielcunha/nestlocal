...
async function resolveOrganization(storeSlug){
  ...
  const settings=await db.collectionGroup('nestlocal_settings').where('slug','==',s).limit(3).get();
  const doc=settings.docs.find(x=>x.data()?.published===true);if(!doc)return null;
  return {id:doc.ref.parent.parent.id,...doc.data()};
}
...
app.post('/api/public/stores/:storeSlug/requests',async(req,res)=>{
  try{
    ...
    const body=req.body||{};const name=clean(body.name).slice(0,100),customerPhone=phone(body.phone),serviceId=clean(body.serviceId),coverageCode=slug(body.coverageCode),quantity=Number(body.quantity);
    if(name.length<2||customerPhone.length<10||!serviceId||!coverageCode||!Number.isSafeInteger(quantity)||quantity<1||quantity>10||body.acceptedTerms!==true)return sendError(res,400,'INVALID_REQUEST');
...
app.get('/api/session',authenticate,async(req,res)=>{
  try{const user=await db.doc(`users/${req.identity.uid}`).get();if(!user.exists)return sendError(res,403,'USER_NOT_FOUND');const data=user.data();let ids=[data.organizationId,data.primaryOrganizationId,data.activeOrganizationId,...(Array.isArray(data.organizations)?data.organizations:[])].filter(x=>typeof x==='string');const [legacy,canonical]=await Promise.all([db.collection('organization_members').where('uid','==',req.identity.uid).limit(50).get(),db.collectionGroup('members').where('uid','==',req.identity.uid).limit(50).get()]);ids.push(...legacy.docs.filter(x=>!inactive(x.data())).map(x=>x.data().organizationId).filter(Boolean),...canonical.docs.filter(x=>!inactive(x.data())).map(x=>x.ref.parent.parent?.id).filter(Boolean));if(globalRoles.has(data.systemRole)){const all=await db.collection('organizations').limit(50).get();ids.push(...all.docs.map(x=>x.id))}ids=[...new Set(ids)];const docs=await Promise.all(ids.map(id=>db.doc(`organizations/${id}`).get()));res.json({user:{uid:req.identity.uid,displayName:data.displayName||req.identity.name||'',systemRole:data.systemRole||'user'},organizations:docs.filter(x=>x.exists&&!inactive(x.data())).map(x=>({id:x.id,name:x.data().name||x.id,slug:x.data().slug||''}))})}catch(e){console.error(e);sendError(res,500,'INTERNAL_ERROR')}});
...
app.put('/api/organizations/:orgId/nestlocal/settings',authenticate,authorize,async(req,res)=>{
  const b=req.body||{};const data={businessName:clean(b.businessName).slice(0,100),slug:slug(b.slug),whatsapp:phone(b.whatsapp),coverageCodes:Array.isArray(b.coverageCodes)?[...new Set(b.coverageCodes.map(slug).filter(Boolean))].slice(0,30):[],validForMinutes:Number(b.validForMinutes||30),updatedAt:admin.firestore.FieldValue.serverTimestamp()};if(data.businessName.length<2||data.slug.length<3||!data.coverageCodes.length||!Number.isSafeInteger(data.validForMinutes)||data.validForMinutes<5||data.validForMinutes>1440)return sendError(res,400,'INVALID_SETTINGS');const collision=await db.collectionGroup('nestlocal_settings').where('slug','==',data.slug).limit(2).get();if(collision.docs.some(x=>x.ref.parent.parent.id!==req.access.orgId))return sendError(res,409,'SLUG_TAKEN');await db.doc(`organizations/${req.access.orgId}/nestlocal_settings/public`).set(data,{merge:true});res.json({ok:true})});
