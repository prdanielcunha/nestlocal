...
app.get('/api/organizations/:orgId/nestlocal',authenticate,authorize,async(req,res)=>{
  try{const monthId=new Date().toISOString().slice(0,7);const [settings,services,requests,customers,usage,members,outbox,revenueMetrics]=await Promise.all([db.doc(`organizations/${req.access.orgId}/nestlocal_settings/public`).get(),db.collection(`organizations/${req.access.orgId}/nestlocal_services`).get(),db.collection(`organizations/${req.access.orgId}/nestlocal_requests`).orderBy('createdAt','desc').limit(100).get(),db.collection(`organizations/${req.access.orgId}/nestlocal_customers`).limit(100).get(),db.doc(`organizations/${req.access.orgId}/nestlocal_usage/${monthId}`).get(),db.collection(`organizations/${req.access.orgId}/members`).limit(100).get(),db.collection(`organizations/${req.access.orgId}/nestlocal_message_outbox`).orderBy('createdAt','desc').limit(30).get(),db.doc(`organizations/${req.access.orgId}/nestlocal_metrics/revenue`).get()]);const team=members.docs.filter(x=>!inactive(x.data())).map(x=>{const d=x.data(),role=clean(d.role||d.organizationRole).toLowerCase(),owner=role==='owner';return{uid:x.id,name:clean(d.displayName||d.name||d.email||x.id),email:clean(d.email),role,nestlocalEnabled:owner||d.appAccess?.nestlocal?.enabled===true,owner}});res.json({organization:{id:req.access.orgId,name:req.access.org.name},entitlement:{...req.access.entitlement,usage:{monthId,requests:Number(usage.data()?.requestCount||0)},seats:{used:team.filter(x=>x.nestlocalEnabled).length,limit:req.access.entitlement.limits.users}},settings:settings.exists?settings.data():null,services:services.docs.map(x=>({id:x.id,...x.data()})),requests:requests.docs.map(x=>({id:x.id,...x.data(),trackingTokenHash:undefined})),customers:customers.docs.map(x=>({id:x.id,...x.data()})),customerCount:customers.size,team,messageOutbox:outbox.docs.map(x=>({id:x.id,...x.data()})),revenueMetrics:revenueMetrics.exists?revenueMetrics.data():{assistedRevenueCents:0,assistedJobs:0}})}catch(e){console.error(e);sendError(res,500,'INTERNAL_ERROR')}});
...
app.put('/api/organizations/:orgId/nestlocal/settings',authenticate,authorize,async(req,res)=>{
  const b=req.body||{},timezone=clean(b.timezone||'America/Sao_Paulo'),workingDays=Array.isArray(b.capacityWorkingDays)?[...new Set(b.capacityWorkingDays.map(Number))].sort((a,b)=>a-b):[],windows=Array.isArray(b.capacityWindows)?[...new Set(b.capacityWindows.map(x=>clean(x)))]:[];
  ...
  const data={businessName:clean(b.businessName).slice(0,100),slug:slug(b.slug),whatsapp:phone(b.whatsapp),coverageCodes:Array.isArray(b.coverageCodes)?[...new Set(b.coverageCodes.map(slug).filter(Boolean))].slice(0,30):[],validForMinutes:Number(b.validForMinutes||30),timezone,published:false,'capacity.workingDays':workingDays,'capacity.windows':windows,'messaging.templates.serviceUpdate':clean(b.whatsappServiceTemplate).slice(0,120),'messaging.templates.maintenanceReminder':clean(b.whatsappMaintenanceTemplate).slice(0,120),updatedAt:admin.firestore.FieldValue.serverTimestamp()};
  if(data.businessName.length<2||data.slug.length<3||!data.coverageCodes.length||!Number.isSafeInteger(data.validForMinutes)||data.validForMinutes<5||data.validForMinutes>1440)return sendError(res,400,'INVALID_SETTINGS');
  await db.doc(`organizations/${req.access.orgId}/nestlocal_settings/public`).set(data,{merge:true});res.json({ok:true})
});
...
app.post('/api/organizations/:orgId/nestlocal/messages/prepare',authenticate,authorize,async(req,res)=>{
  try{
    ...
    await ref.create({category,targetType:requestId?'request':'customer',targetId,channel:'whatsapp',provider:'whatsapp_cloud_api',status,blockReason:eligibility.eligible?'':eligibility.reason,templateName,sendMode:'approved_template_required',preparedBy:req.identity.uid,createdAt:admin.firestore.FieldValue.serverTimestamp(),updatedAt:admin.firestore.FieldValue.serverTimestamp()});
